...
def publishVelocity(v_list):
    ...
    while not rospy.is_shutdown():
        twist.linear.x = v_list[0]; twist.linear.y = v_list[1]; twist.linear.z = 0.0
        twist.angular.x = 0.0; twist.angular.y = 0.0; twist.angular.z = v_list[2]
...
def action(node, canvas, vel_l, vel_r, R, L):    # Local angles
    ...
    velocity_list.append((0.5*R*(vel_l+vel_r)*np.cos(np.deg2rad(theta_init)), -0.5*R*(vel_l+vel_r)*np.sin(np.deg2rad(theta_init)), (-R/L)*(vel_r - vel_l)))
...
if __name__ == '__main__':
    ...
    clearance, robot_radius, vel_l, vel_r = 20, 0, 5, 50 
    ...
    initial_state, final_state = [50, 50, 0], [900, 900]
    
    #Converting the RPMs to Radians/second
    # vel_l = np.pi*vel_l/60
    # vel_r = np.pi*vel_l/60
